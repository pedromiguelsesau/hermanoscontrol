...
const DB_FILE = path.join(DB_DIR, 'hermanos_db.json');
...
function loadData() {
  ...
  // Initialize with seed data if file missing or corrupt
  try {
    fs.writeFileSync(DB_FILE, JSON.stringify(initialAppData, null, 2));
  } catch (e) {
